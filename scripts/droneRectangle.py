...
class droneRectangle():
    ...
    def __init__(self):
        ...
        PlanetaryRadius=6371.0#units in km
        self.CoordConverter=lambda t: (str((-t[3])+(t[1]*((t[2]) / 2.0))/(math.pi*PlanetaryRadius*math.cos(((t[4])+(t[0]*((t[2]) / 2.0))/    \
            (math.pi*PlanetaryRadius / 180.0))*math.pi/180.0) / 180.0)),str((t[4])+t[0]*((t[2]) / 2.0)/(math.pi*PlanetaryRadius / 180.0)))
    ...
    def giveRotFootprint(self, Rotation: float, Diameter: float, c1mid: float, c2mid: float) -> str:
        ...
        coordsMap=[self.CoordConverter([*coordClosed,Diameter,c1mid,c2mid]) for coordClosed in coordsClosed]
